...
/// </summary>
public sealed class PacketCatalog(string name = "unknown")
{
    ...
    {
        writer.WriteLine($"\n{'─',40}");
        writer.WriteLine($" [{name}] s2c Catalog — {_entries.Count} 個不同 opcode");
        writer.WriteLine($"{'─',40}");
    ...
    public string ToJson()
        => JsonSerializer.Serialize(
            ...
            {
                source  = name,
                packets = _entries.Values.OrderBy(e => e.Opcode).Select(e => new
                {
