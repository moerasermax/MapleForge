...
function Shot([IntPtr]$h,[string]$t){ [WJ+R]$r=New-Object 'WJ+R'; [void][WJ]::GetWindowRect($h,[ref]$r); $w=$r.Ri-$r.L;$ht=$r.B-$r.T; if($w-le 0){return}; $b=New-Object System.Drawing.Bitmap($w,$ht);$gg=[System.Drawing.Graphics]::FromImage($b);$gg.CopyFromScreen($r.L,$r.T,0,0,(New-Object System.Drawing.Size($w,$ht)));$b.Save("$Shot\rec-$t.png",[System.Drawing.Imaging.ImageFormat]::Png);$gg.Dispose();$b.Dispose() }
...
try{
    ...
    # 點登入鈕 → 連拍「錄影」彈回過程(每 350ms 一張，~28 張≈10 秒)，
    # 捕捉登入成功→彈回瞬間有沒有閃過錯誤框/PIN/世界選擇。
    Write-Host "點登入鈕 → 連拍錄影中..."
    [WJ]::Click($g,0.78,0.46)
    for($f=0;$f -lt 28;$f++){ Shot $g ("frame-{0:D2}" -f $f); Start-Sleep -Milliseconds 350 }
    Write-Host "連拍完成(rec-frame-00..27)"
