...
突破前只有 server 端擷取（半把，只聽得到 c2s）。現在 windower 客戶端側 oracle 修通，**兩個方向都打通**，而且證明了「不破壞客戶端 + 真實資料端到端解碼到位元級」——擷取工具最難的一關（零干擾觀測）。

| 路徑 | 聽得到 | 用途 |
|---|---|---|
| server 端擷取 | c2s（客戶端→server） | 客戶端送的每個封包，有 ground truth |
| **windower（新解鎖）** | **雙向，尤其 s2c（server→客戶端）** | **server 回的每個封包，含我們還不會做的那半邊** |

### 能施展的空間

1. **接「原始爸爸的 server」錄 s2c** → 學 authoritative server→client 協定（登入回應、角色列表、進地圖、怪物生成、掉落、技能、封包簽章…）。[3] 硬骨頭從「不可能」變「可做」。
2. **建協定 fixture 庫** → 每種封包都有真實位元級黃金樣本，測試驅動乾淨框架重構；對錯由「位元級真相」說了算。
3. **差異定位** → 我們 server 的 s2c 輸出 vs 原始 server 逐 byte diff，精準找到重構分歧點。
4. **任何協定疑問的第一直覺** → 抓一次真實交握 → 離線解 → 斷言，不必再讀敏感舊原始碼逆推。

### 解鎖的新技能（能力清單）

- **雙向真實封包擷取**（c2s 有 ground truth + s2c authoritative 學習）
- **客戶端 instrumentation 基礎建設**：穩健 winsock hook + hotpatch/LDE detour 引擎（之後 hook 別的 API 可複用）
- **overlapped/IOCP 完成路徑擷取**（三條路都備好，雖此客戶端走同步 recv）
- **一套 live 除錯方法論 + 5 支診斷 harness**：A/B 隔離 → 逐函式隔離 → 讀碼定罪
- **「零干擾觀測」工程戒律**：高頻路徑的同步 I/O 是隱形殺手

### 誠實的邊界

- s2c 真正價值要**接到原始 server** 才釋放（接自己的 server 只看到已知的）。
- 完整無人值守登入流程仍有 GUI 細節要打磨（但**登入本身已通**）。
- Themida 脆弱性仍在（已接受）。
