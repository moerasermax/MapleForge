...
5. **注入穩定性(必做，否則偶爾帳密錯誤)**：一次性 `"testuser\ttest1234"` 高速連發會偶爾掉字/錯位(密碼框混入多餘字元→帳密錯誤)。穩定化=**每欄位注入前先送一串 Backspace 清空**(建立確定起點，不靠記憶帳號反白)、帳密**分次注入**、Tab 獨立步驟。實測穩定化後**連續 2 次乾淨登入成功**(2026-06-02 10:34/10:36)。
