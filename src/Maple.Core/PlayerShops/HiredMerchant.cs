...
public sealed class HiredMerchant : IPlayerShop
{
    public PlayerShopState State { get; set; } = new() { Kind = PlayerShopKind.HiredMerchant };
    ...
    public static HiredMerchant Create(
        int ownerId,
        int ownerAccountId,
        string ownerName,
        int itemId,
    ...
    public static HiredMerchant FromState(PlayerShopState state)
    {
        state.Kind = PlayerShopKind.HiredMerchant;
        return new HiredMerchant { State = state };
    }

    public bool IsOwner(int characterId, string name) => State.IsOwner(characterId, name);
    public bool IsExpired(DateTimeOffset now) => State.IsExpired(now);
