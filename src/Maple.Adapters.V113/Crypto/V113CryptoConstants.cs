namespace Maple.Adapters.V113.Crypto;

/// <summary>
/// v113 加密常數，逐 byte 取自舊 Java <c>tools/MapleAESOFB.java</c>（Vana/OdinMS 變體）。
/// 任何一個 byte 錯誤都會導致客戶端無法連線，故與來源並列以利核對。
/// </summary>
internal static class V113CryptoConstants
{
    public const short MapleVersion = 113;

    /// <summary>256-bit AES 金鑰（每 4 bytes 實際只用第 1 byte，其餘為 0）。</summary>
    public static readonly byte[] AesKey =
    {
        0x13, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00,
        0x06, 0x00, 0x00, 0x00, 0xB4, 0x00, 0x00, 0x00,
        0x1B, 0x00, 0x00, 0x00, 0x0F, 0x00, 0x00, 0x00,
        0x33, 0x00, 0x00, 0x00, 0x52, 0x00, 0x00, 0x00,
    };

    /// <summary>IV 演化（funnyShit）用的 256-byte 置換表。順序同 MapleAESOFB.java 48–64 行。</summary>
    public static readonly byte[] FunnyBytes =
    {
        ,
    };
}
